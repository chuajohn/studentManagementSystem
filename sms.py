...
def export_data():
    ...
    table=pandas.DataFrame(newlist,columns=['Id','Name','Email','Address','Gender','DOB','Added Date','Added Time'])
    table.to_csv(url,index=False)
    messagebox.showinfo('Success','Data is saved succesfully')

def toplevel_data(title,button_text,command):
    global idEntry,phoneEntry,nameEntery,emailEntry,addressEntry,genderEntry,dobEntry,screen
    screen= Toplevel()
    screen.title(title)
    screen.grab_set()
    ...
    emaildobtry.grid(row=3, column=1, pady=15, padx=10)
    ...
    if title=='Update Student':
        ...
        emailEntryEntry.insert(0, listdata[3])
        addressEntryEntry.insert(0, listdata[4])
        genderEntryEntry.insert(0, listdata[5])
        dobEntryEntry.insert(0, listdata[6])
...
def connect_database():
    ...
    connectWindow.geometry('470x250+230')
...
sliderLabel=Label(root,font=('arial',28,'italic bold'),width=36)
